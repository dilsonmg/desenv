...
$caminho_arquivo = 'd:/desenv/php/expedicao/arquivostst/mprimas.csv';
...
$rs2 = mysql_query("select a.* , DATEDIFF(a.data_venc,CURDATE()) dias_avencer,
                    ...
                    left outer join tv_ttsaidmatp v on v.cod_prod = a.cod_prod and v.num_lote = a.num_lote 
					 " . $p11  . $p21 ."
                     where a.id_entmatp > 0 and a.cod_prod not in (100010,100011,100012) " . $p3 .$p31 . "
					 group by a.cod_prod, a.num_lote
                    order by a.cod_prod asc ");				  
    $b = mysql_num_rows($rs2);
	
/*	
$rs2txt = mysql_query("select a.cod_fornec,c.rz_social,a.cod_prod,b.descr_prod,a.unidade,a.num_nf,
                       DATE_FORMAT(a.data_nf, '%d/%m/%Y') data_nff,a.num_lote,DATE_FORMAT(a.data_fab, '%d/%m/%Y') data_fabr,
					   DATE_FORMAT(a.data_venc, '%d/%m/%Y') data_venc,a.atv_kamoran,
					   if(d.sald_lote is null,d.tt_entradalote,d.sald_lote) saldo,	
					   a.motivo_ent
                    from tb_entmatp a  
                    inner join tb_produto b on a.cod_prod = b.cod_prod " . $p4 . "
                    inner join tb_fornecedor  c on c.cod_fornec = a.cod_fornec
                    inner join tv_ttsaidalote d on d.cod_prod = a.cod_prod and d.num_lote = a.num_lote
					 ...
					 and d.sald_lote > 0
					 union all
					 select a.cod_fornec,c.rz_social,a.cod_prod,b.descr_prod,a.unidade,a.num_nf,
                       DATE_FORMAT(a.data_nf, '%d/%m/%Y') data_nff,a.num_lote,DATE_FORMAT(a.data_fab, '%d/%m/%Y') data_fabr,
					   DATE_FORMAT(a.data_venc, '%d/%m/%Y') data_venc,a.atv_kamoran,
					   if(d.sald_lote is null,d.tt_entradalote,d.sald_lote) saldo,	
					   a.motivo_ent
                    ...
                    inner join tv_ttsaidalote d on d.cod_prod = a.cod_prod and d.num_lote = a.num_lote
	                 where a.id_entmatp > 0 and a.cod_prod not in (100010,100011,100012) " . $p3 .$p31 . "
					 and d.sald_lote is null
					 group by a.cod_prod, a.num_lote
                    order by cod_prod asc INTO OUTFILE 'd:/desenv/php/expedicao/arquivostst/mprimas.csv'");		
					
					$fp = fopen("./arquivostst/data.txt", "w");
...
function abrir_csv(arquivo){ 
   //       var setor_doc = document.form1.setor_doc.value
	      urln = './arquivostst/'+arquivo;
		  		    //alert(setor_dc);

          newwindow=window.open(urln,'arquivo','top=10,left=200,location=0,directories=0,toolbar=no, scrollbars=yes, resizable=yes,height=600,width=900');
}
...
<table width="100%" border="0">
      <tr>
        ...
        <th align="right"><img src="../imagens/tecladoclaro.png" ><br><a  href=javascript:window.print()><img border="0" src="../imagens/print.png"    title="Imprimir"></a></th>
      </tr>
      <tr>
      ...
      <tr>
        <th colspan="3" align="center">M. Prima
          <input type="text" id = "mprimaps" name="mprimaps"  maxlength="40" size="40"  class="search-input3" >
          Tipo
...
//echo($b);
	 if ($b > 0){
	  ...
	  $tt_soma = 0;
	  ?>
	     <tr><th colspan=3  align="right"><a href="javascript:abrir_csv('contr_venc.csv')"><h2> Editar - controle de vencimentos.csv</</a></th></tr>
              ...
              <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr bgcolor="#D2D2FF" >
                  ...
                  <th colspan=2 align="right">Ativo Kamoran</th>
                  <th>Saldo KG</th>
                  <th>Tipo</th>
                </tr>
                <tr bgcolor="#D2D2FF" >
	...
	<?php
//     $ins=mysql_query( $rs2txt );
	$fp = fopen("./arquivostst/contr_venc.csv", "w");
			$escreve = fwrite($fp,"Fornecedor;codigo;produto;unidade;N. Nf;Data Nf;N. Lote;Dt. Fabr;Dt. Venc; Atv. Kamoran; Saldo Kg;Tipo");
			$escreve = fwrite($fp," \n");
   //fclose($fp);
	  ...
	  if($row['tt_lote'] > 0 ){
       ...
       if ($row['cod_prod'] != $cd_prodini && $tot_prod > 0){  ?>
                <tr>
                  <th colspan="10" align="right" bgcolor="#E8E8E8" >T O T A L </th>
                <th align="right" bgcolor="#E8E8E8" ><b><?php  echo (number_format($tot_prod,3,',',''));?></b></th>
                <th align="right" bgcolor="#E8E8E8" >&nbsp;</th>
                </tr>
           <?php 
			   $tot_prod = 0.00;
			} 
			$escreve = fwrite($fp,$row['rz_social'] .";");
			$escreve = fwrite($fp,$row['cod_prod'] .";");
			$escreve = fwrite($fp,$row['descr_prod'] .";");
			$escreve = fwrite($fp,$row['unidade'] .";");
			//$escreve = fwrite($fp,$row['num_nf'] .";");
			//$escreve = fwrite($fp,$row['data_nff'] .";");
			$escreve = fwrite($fp,$row['num_lote'] .";");
			$escreve = fwrite($fp,$row['data_fab'] .";");
			$escreve = fwrite($fp,$row['data_venc'] .";");
			$escreve = fwrite($fp,$row['atv_kamoran'] .";");

		
			?>
              <tr>
                <th align="left" >
           <?php echo ($row['rz_social']);?></th>
                <th align="left" ><?php echo ($row['cod_prod']);?></th>

           <th align="left" ><?php echo ($row['descr_prod']);?></th>
		  ...
		  */
		   ?></th>
           <!--th align="center" ><?php //echo ($row['data_nff']);?></th-->
           <th align="center"><?php echo ($row['num_lote']);?></th>
           <th align="center"><?php echo (strftime("%d/%m/%Y", strtotime($row['data_fab'])));
		   ...
		   ?></th>
           <th align="center"><?php echo ($row['atv_kamoran']); 
		   		   
		   ?></th>
           <th align="right"><?php echo ($row['tt_lote']); 
		   		   $tt_soma = $tt_soma + $row['tt_lote']; 
				   $tot_prod = $tot_prod + $row['tt_lote']; 
		   ?></th>
           <th align="center"><?php echo ($row['descr_subgr']);?></th></tr>
          <?php 
		  			$escreve = fwrite($fp,$row['tt_lote'] .";");

		  			$escreve = fwrite($fp,$row['descr_subgr'] .";");
			$escreve = fwrite($fp," \n");
		 ...
		 fclose($fp);
		  ?>     
              <tr bgcolor="#E1E1FF">
                <th colspan="10" align="right" >T O T A L </th>
                <th align="right"><b><?php echo (number_format($tot_prod,3,',',''));?></b></th>
                <th align="right">&nbsp;</th>
              </tr>
               ...
               <tr>
                ...
                <th align="right"><b><?php echo (number_format($tt_soma,3,',','.'));?></b></th>
                <th align="right">&nbsp;</th>
               </tr>
