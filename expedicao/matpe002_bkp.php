...
//$rs33=mysql_query(" select a.*, (a.quantid_ent - a.quant_said) saldo
 ...
 $cria_tab = " create table tmp_said as
				select a.cod_prod, a.num_lote, a.quantid_ent,
				...
				from tb_entmatp a where a.cod_prod = '".$cod_prod."'";
...
$lm = "";					
 if (!isset($id)){
				/*	
					...
					*/		
					$rs33a = mysql_query("select a.* from tmp_said a where a.cod_prod = '".$cod_prod. "' and a.num_lote = '" .$num_lote . "'");			  	
...
<table width="99%" border="0">
	...
	echo($_SESSION['id_entmatp']);
      ...
      <tr>
        ...
        <th colspan="2" align="left"><select name="cod_prod"  class="search-input" <?php if($habilita ==1) echo(" disabled ");?> onChange="atualiza();">
...
//		
      if ($lm == "") { ?>
        <select name="num_lote"  <?php if($habilita ==1) echo(" disabled ");?> onChange="atualiza();" class="search-input3">
          ...
          <?php while($row33=mysql_fetch_assoc($rs33) ){ 
		     if( $row33['saldo'] > 0){  ?>
                 <option value="<?php echo($row33['num_lote'])?>"
				  ...
				  ?>	
				 ><?php echo($row33['num_lote']);?></option>
          <?php 
		   }
		  }?>
        </select>
      ...
      <tr>
        ...
        <select name="motivo" class="search-input6" >
	 ...
	 if ($b > 0){
	  ...
	  while($row=mysql_fetch_array($rs2)){ 
	   ...
	   echo('<tr ' . $bgc .'>');?>
              ...
              <tr>
                <td >
           <a href="matpe002_bkp.php?id=<?php echo ($row['id_saidmat']);?>"><?php echo ($row['cod_prod'] . " - " . $row['descr_prod'] );?></a></td>
