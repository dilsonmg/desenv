...
if(isset($lote_fabr22)){
	if ($lote_fabr22 <> "" ){
		$p01 = " and a.num_lote like '%" . $lote_fabr22 ."%'";
  }
}
...
from tb_entprodac a
 inner join tb_produto b on a.cod_prod = b.cod_prod 
  where a.id_entprodac > 0 " . $p01 . "
 order by a.id_entprodac desc limit 200");				  
    $b = mysql_num_rows($rs2);

$rs33 = mysql_query("select a.* from tb_produto a 
where a.id_prod > 0 and trim(a.linha) not in ('CONSUMO','BRINDES','OUTROS') 
...
if (isset($id)){
    ...
    $a = mysql_num_rows($rs1);
     ...
     if ($a > 0 ) {
         $habilit = "N";
         $rs1 = mysql_query("SELECT a.* ,DATE_FORMAT(a.data_liblote, '%d/%m/%Y') data_liblote1
FROM tb_entprodac a where a.id_entprodac =". $id);
    	 $row1 = mysql_fetch_assoc($rs1);

	     $data_prevlib    = strftime("%d/%m/%Y", strtotime($row1['data_prevlib']));
	     $data_liblote    = $row1['data_liblote1'];
		 
		 
		 
//echo("data lib ".$data_liblote);		 
...
<input readonly type=hidden name=x size=3 maxlength=3 value="950">
...
<table width="98%" border="0">
	...
	echo($_SESSION['id_entmatp']);
        ...
        <th align="right"><img src="../imagens/tecladoclaro.png" ><br>
         <?php echo( $_SESSION['nome_usu'] . " - " . $hoje); ?>
        </th>
      </tr>
      </table>
 <table width="98%" border="0">
      ...
      <tr>
        ...
        <th align="left"><input type="text" id = "num_lote" required name="num_lote"  maxlength="20" size="20" placeholder="informe N.lote"  value="<?php echo($num_lote); ?>" class="search-input5"></th>
      </tr>
      <tr>
        ...
        <th align="left"><input type="text" id = "data_venc" required name="data_venc"  maxlength="20" size="20" placeholder="informe vencimento"  value="<?php echo($data_venc); ?>" class="search-input5"
      ...
      <tr>
        ...
        <input type="text" name="data_prevlib" size="8" maxlength="10"  placeholder="Informe no formato 99/99/9999" value="<?php echo $data_prevlib ?>" title="Informe no Formato 99/99/9999" onKeyPress="mascara(this)" onBlur="verifica_data(this.value,data_prevlib);" onChange="verifica_data(this.value,data_prevlib);" class="search-input5"/>
        Tipo de Entrada<br>
       ...
       <tr>
        ...
        <th colspan="4" align="left">  
          ...
          <textarea name="msg_lote" cols="105" rows="4"  onchange="textCounter(this.form.msg_lote,this.form.x,930);"  onKeyDown="textCounter(this.form.msg_lote,this.form.x,930);"  ><?php echo($msg_lote) ?></textarea></td>
        <!--input type="submit" name="gravaobs"  value="Gravar Obs"  class="search-submit2" onClick="validamatpacb();"/-->
      ...
      <tr>
        <th colspan="4" align="center"  bgcolor="#8080FF">&nbsp;</th>
      </tr>
      <tr>
        <th colspan="4" align="center">
        <?php
           if ($row['data_liblote'] == "00/00/0000" ||$data_liblote == "" || $_SESSION['permi'] == '999' ) {
		?>
          <input type="button" name="gravar"  onClick="validamatpacb();" value="Gravar"  class="search-submit2" />
        <?php } ?>  
          <input type="button" name="button" id="button" value="Limpar Formulario" onclick="resetForm();"  class="search-submit2">
        <?php
           if ($data_liblote == "00/00/0000" || $data_liblote == "" || $_SESSION['permi'] == '999' ) {
		?>
        ...
        Lote Fabricado 
          <input type="text" id = "lote_fabr22" required name="lote_fabr22"  maxlength="20" size="20" placeholder="informe N.lote"  value="" class="search-input3">
              <input name="Pesquisar" type="button" value="Pesquisar"  class="search-submit2"  onClick="pesquisacpo();" >
           </th>   
        </tr>
            <tr >
	 ...
	 if ($b > 0){
	  ...
	  while($row=mysql_fetch_array($rs2)){ 
	   ...
	   echo('<tr ' . $bgc .'>');?>
           ...
           <?php  if ($row['data_liblote'] == "00/00/0000" ||$row['data_liblote'] == "" || $_SESSION['permi'] == '999' || $_SESSION['idusuario'] == '84' || $_SESSION['idsetor'] == '22') { ?>  
