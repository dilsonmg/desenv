...
if (isset($data_1)){
	if ($data_1 <> "" ){
		 $p1a = " and a.venci_retencao >= '" . formata_data2($data_1) ."'" ; 
	}
}
...
if (isset($data_2)){
	if ($data_2 <> "" ){
		 $p1b = " and a.venci_retencao <= '" . formata_data2($data_2) ."'" ; 	
		 }
}
...
if($cod_prod == ""){$p1 = "";}
 ...
 $rs2 = mysql_query("select a.*,b.descr_prod
					,DATE_FORMAT(a.data_fabr, '%d/%m/%Y') data_fabr1,
					 ...
					 DATE_FORMAT(a.data_fabr, '%d/%m/%Y') data_fabr1,
					 DATE_FORMAT(last_day(a.venci_retencao), '%d/%m/%Y') venci_retencao1,
					 DATEDIFF(last_day(a.venci_retencao),CURDATE()) dias_avencer					 
					from tb_contraprovm a
					inner join tb_produto b on a.cod_prod = b.cod_prod " . $p2 . 
					"where a.id_contraprovm > 0 and a.cod_prod not in (100010,100011,100012) " . $p1 . $p01 . $p1a . $p1b ."  and a.situacao = 'A' order by a.venci_retencao asc");				  
...
<![endif]-->
	<title>MATCPCVM1 - CONSULTA VENCIMENTO - CONTRAPROVA MATERIA PRIMA</title>
...
<table width="100%" border="0">
     ...
     <th align="right"><h1>Consulta por Vencimento Contraprova - Materia Prima- 
        <?php
...
<a  href=javascript:window.print()><img border="0" src="../imagens/print.png"    title="Imprimir"></a>        </th>
      ...
      <tr>
        <th colspan="4" align="center"><input type="button" onClick="sair();" value="Sair" class="search-submit2">
        </th>
        </tr>
            <tr >
              <th colspan="4" align="center"  bgcolor="#8080FF">Registros Arquivados</th>
            </tr>
            <tr >
              <th colspan="4" align="center">M.P
                <input type="text" name="pa_pesq" id="pa_pesq" maxlenght="50" size=40 /> 
              </h1>Lote M.P 
              <input type="text" id = "lt_fabr2"  name="lt_fabr2"  maxlength="45" size="40" placeholder="informe o lote !"  value="">
              Periodo
...
a
<input type="text" name="data_2" size="10" maxlength="10"  title="Informe no Formato 99/99/9999" onkeypress="mascara(this)" onblur="verifica_data(this.value,data_2);"/><input name="Pesquisar" type="submit" value="Pesquisar" class="search-submit2">
              </th>
            </tr>
            <tr align="center">
	   ...
	   echo('<tr ' . $bgc .'>');?>
         ...
         <?php echo ($row['cod_prod'] . " - " . $row['descr_prod']);?></td>
		   ...
		   <?php 
              ...
              if ($row['venci_retencao'] == ""){
				   echo ("Indeterminado !"); }
			  else{
		   		//   echo (strftime("%d/%m/%Y", strtotime($row['venci_retencao1'])));
				     echo ($row['venci_retencao1']);
				  
				   if($row['dias_avencer'] < 0){
                          echo ('<font color="#FF0000"> - Descartar em ' . $row['dias_avencer'].' dias </font>');}
                   else {
			              echo ('<font color="#0000FF"> - Descartar em ' . $row['dias_avencer'].' dias </font>'); 
		           }
				}
			?>
