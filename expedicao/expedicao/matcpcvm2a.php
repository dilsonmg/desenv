...
if (isset($data_1)){
	if ($data_1 <> "" ){
		 $p1a = " and c.data_entrada >= '" . formata_data2($data_1) ."'" ; 
	}
}
...
if (isset($data_2)){
	if ($data_2 <> "" ){
		 $p1b = " and c.data_entrada <= '" . formata_data2($data_2) ."'" ; 	
		 }
}
...
if($cod_prod == ""){$p1 = "";}
 ...
 $rs2 = mysql_query("select a.*,b.descr_prod
					,DATE_FORMAT(a.data_fabr, '%d/%m/%Y') data_fabr1,
					 ...
					 DATE_FORMAT(c.data_entrada, '%d/%m/%Y') data_entrada
					 from tb_contraprovm a
					inner join tb_produto b on a.cod_prod = b.cod_prod " . $p2 . 
				   "inner join tb_entmatp c on c.cod_prod = a.cod_prod and c.num_lote = a.num_lote " . $p1a . $p1b . "
					where a.id_contraprovm > 0 " . $p1 . $p01 ."  and a.situacao = 'A' order by c.data_entrada desc");				  
...
<![endif]-->
	<title>MATCPCV1 - CONSULTA VENCIMENTO - CONTRAPROVA MATERIA PRIMA</title>
...
<table width="95%" border="0">
      <tr>
        ...
        <th align="center"><h1>Contraprova - Consulta por Data Entrada - Materia Prima- 
        <?php
