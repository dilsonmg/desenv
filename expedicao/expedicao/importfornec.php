...
<p><img src="../imagens/banner2.jpg" >
 ...
 <br />
 Importa&ccedil;&atilde;o de dados - Fornecedores
...
/*================================ importar dados de produtos ==============================*/
        $rs1 = mysql_query("SELECT * FROM tb_fornecedor where cod_fornec = '". $data[0] ."'");
        $a   = mysql_num_rows($rs1);
	    //printf($a);
		$rz_social = str_replace("'","`",$data[1]);	
		$rz_social = str_replace("€","C",$rz_social);	
        $rz_social = str_replace("§",".",$rz_social);
		$cod_fornec = $data[0];
        if ($a > 0) {
			$altera = "update tb_fornecedor set rz_social = '" . $rz_social . "'" . 		 
			" where cod_fornec = '" . $data[0] . "'";
			//echo("update tb_produto set descr_prod = '" . $descr_prod . "'" . 		 
			//" where cod_prod = '" . $data[0] . "'");
			mysql_query($altera) or die(mysql_error());
			
		}else{	
          //  $rs2   = mysql_query("SELECT max(id_fornec) + 1 as id_fornec FROM tb_fornecedor ");
        //	$row1  = mysql_fetch_assoc($rs2);
	     //   $nvid  = $row1['id_fornec'];
		//	if ($nvid == ""){$nvid = 1; }
			$inclui = " insert into tb_fornecedor (id_fornec, rz_social,cod_fornec) 
			values ('$cod_fornec','$rz_social',$cod_fornec)";
