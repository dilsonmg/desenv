...
if(isset($ano_i) && $ano_i > 0 ){
	$anodf = $ano_i; }
    $p1 = " and year(a.data_liblote) = '" . $anodf ."'";
	
	
if($mes_i > 0){
	$p2 = " and month(a.data_liblote) = '" .$mes_i ."'";
}
...
$rs_tmp = "create table tmp_saldoprd as (SELECT a.cod_prod,
sum(a.tt_lote) saldo_prod
...
/*
SELECT a.cod_prod,b.descr_prod,year(a.data_liblote) ano,month(a.data_liblote) mes, sum(a.quant_fabr) tt_mes
 FROM tb_entprodac a
inner join tb_produto b on b.cod_prod = a.cod_prod
where year(a.data_liblote) = 2019
group by a.cod_prod, month(a.data_liblote),year(a.data_liblote) order by year(a.data_liblote),a.cod_prod,mes;	

	*/			
		
							   					
$rs2 = mysql_query("SELECT a.cod_prod,b.descr_prod,year(a.data_liblote) ano,month(a.data_liblote) mes, sum(a.quant_fabr) tt_mes,c.saldo_prod
					 FROM tb_entprodac a
					inner join tb_produto b on a.cod_prod = b.cod_prod
					left outer join tmp_saldoprd c on a.cod_prod = c.cod_prod
					where a.cod_prod > 0 " . $p1 . $p2 ."
					group by a.cod_prod, year(a.data_liblote), month(a.data_liblote)
					order by year(a.data_liblote),a.cod_prod,mes ");			 
	 ...
	 while($row=mysql_fetch_array($rs2)){ 
       ...
       if ( $row['cod_prod']	!= $cd_prod){
	      ...
	      if ($ttm > 0){
			  ...
			  $header2[17] = 'CX';
			  if ($row['cod_prod'] == '1550' ||$row['cod_prod'] == '1551' ||$row['cod_prod'] == '1701' ||$row['cod_prod'] == '8829' ||$row['cod_prod'] == '8831' ) {
			  $header2[17] = 'Kg';}
	          }
		  else { $header2[15] = '';}
	  ...
	  $header2[16] = number_format(($row['saldo_prod']),2,",",".");
