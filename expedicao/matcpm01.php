...
//DATEDIFF(t.data_conserto,CURDATE())
 ...
 $rs32 = mysql_query("select a.cod_prod , b.descr_prod from tb_contraprovm a
 ...
 $rs33 = mysql_query("select a.num_lote from tb_contraprovm a where a.cod_prod = '".$cod_prod ."' and a.situacao = 'A' ");				
...
if (isset($id)){
    ...
    $rs1 = mysql_query("select a.*
										...
										DATE_FORMAT(a.venci_retencao, '%d/%m/%Y') venci_retencao1
										from tb_contraprovm a
                       where a.id_contraprovm  = '". $id ."' and a.situacao = 'A' ");
...
/* 			  
		  ...
		  echo("select a.num_lote from tb_contraprovm a where a.cod_prod = '".$cod_prod .
		  "' and a.num_lote = '". $num_lote ."' and a.situacao = 'A' ");
*/		   

		  $rs33 = mysql_query ("select a.num_lote from tb_contraprovm a where a.id_contraprovm > 0 ".$p1 ." and a.situacao = 'A'  ");
...
if($cod_prod == ""){$p1 = "";}
 ...
 $rs2 = mysql_query("select a.*,b.descr_prod
					,DATE_FORMAT(a.data_fabr, '%d/%m/%Y') data_fabr1,
					 DATE_FORMAT(a.data_venc, '%d/%m/%Y') data_venc1,
					 DATE_FORMAT(a.venci_retencao, '%d/%m/%Y') venci_retencao1,c.rz_social
					from tb_contraprovm a
					inner join tb_fornecedor c on c.cod_fornec = a.cod_fornec
					inner join tb_produto b on a.cod_prod = b.cod_prod " . $p2 . 
					"where a.id_contraprovm > 0 " . $p1 . $p01 ."  and a.situacao = 'A' order by a.id_contraprovm desc");				  
...
<![endif]-->
	<title>MATCPM01 - CONTRAPROVA MATERIA PRIMA</title>
...
function resetForm(){
   // if (confirm("Confirma limpeza do formulário  ?")){
		  ...
		  document.form1.num_lote.value = '';
   	   	  document.form1.action="matcpm01.php";
		  document.form1.submit();  
		  return true;
	//	  }
...
<table width="95%" border="0">
      <tr>
        ...
        <th align="center"><h1>Contraprova - Materia Prima- 
        <?php
...
//		
      ...
      <tr>
        <th colspan="6" align="center">
          <input type="button" name="gravar"  onClick="validacontrprm();" value="Gravar" style="font:color="#006600"-size:8" />
          <input type="button" name="button" id="button" value="Limpar Formulario" onclick="resetForm();" >
          <input type="button" name="Submit4"  onclick="excluircontrprm()(<?php echo($id); ?>);" value="Excluir" style="font:color="#006600"-size:8" />
          <input type="button" onClick="sair();" value="Sair">
        </th>
        </tr>
            <tr >
              <th colspan="6" align="center"  bgcolor="#8080FF">Registros Arquivados</th>
            </tr>
            <tr >
              <th colspan="6" align="center">P.A
	  ...
	  while($row=mysql_fetch_array($rs2)){ 
	   ...
	   echo('<tr ' . $bgc .'>');?>
              ...
              <tr>
                ...
                <td >
           <a href="matcpm01.php?id=<?php echo ($row['id_contraprovm']);?>"><?php echo ($row['cod_prod'] . " - " . $row['descr_prod']);?></a></td>
