...
<p><img src="../imagens/banner2.jpg" >
 ...
 <br /> <br /> <br /> <br />
 Importa&ccedil;&atilde;o de dados - Itens de despesas totais
...
/*================================ importar dados de produtos ==============================*/
        $rs1 = mysql_query("SELECT * FROM tb_despreal where cod_sidespreal = '". $data[0] ."'");
        $a   = mysql_num_rows($rs1);
	    //printf($a);
		$descr_despreal = str_replace("'","`",$data[1]);	
		$descr_despreal = str_replace("€","C",$descr_despreal);	
        $descr_despreal = str_replace("§",".",$descr_despreal);
		$cod_sidespreal = $data[0];
        if ($a > 0) {
			$altera = "update tb_despreal set descr_despreal = '" . $descr_despreal . "'" . 		 
			" where cod_sidespreal = '" . $data[0] . "'";
			mysql_query($altera) or die(mysql_error());
			
		}else{	
			$inclui = " insert into tb_despreal (cod_sidespreal, descr_despreal) 
			values ('$cod_sidespreal','$descr_despreal')";
