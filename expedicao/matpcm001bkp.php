...
if(isset($_POST["botao"])){
		...
		$rs2 = mysql_query("SELECT a.cod_prod,b.descr_prod,b.linha, year(a.data_saida) ano, month(a.data_saida) mes ,
							sum(a.quantid_said) tt_mes,c.saldo_prod, b.unid_mat,b.ponto_min FROM tb_saidmatp a
		...
		$rscons = mysql_query("select a.cod_prod,b.descr_prod, year(a.data_saida) ano,
							sum(a.quantid_said) consumo_ano,avg(a.quantid_said) media_cons,c.saldo_prod,b.ponto_min
							FROM tb_saidmatp a
...
if(isset($_POST["botao"])){
	 ...
	 while($row=mysql_fetch_array($rs2)){ 
	   ...
	   if($ressupr > 0 && $ressupr < 13){
  			$var_graf = $var_graf . "['".trim($header2[1]). "'," . $ressupr. ",'" . $ressupr. "']," ;
	   }
	   /// zera tabela
	   	  for ($i=0;$i < 19; $i++) {
	        $header2[$i] = ''; 
...
function drawChart() {
   ...
   //       ['Item', 'consumo','Saldo','Pto Min'],
            ['Item', 'Meses Suprimento', { role: 'annotation' }], 
	           <?php echo($var_graf);?>     ]);

		var view = new google.visualization.DataView(data);
		
        var options = {
          title : 'Ressuprimento em até 12 meses - Ano de Referencia : <?php echo($anodf); ?>',
          vAxis: {title: 'Itens'},
          hAxis: {title: 'Meses'},
          seriesType: 'bars',
