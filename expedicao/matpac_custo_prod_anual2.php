...
if (isset($ano_2)){
	...
	/*
        ...
        $rs33 = mysql_query("SELECT  b.linha FROM tb_producao1 a
					inner join tb_produto b on a.cod_prod = b.cod_prod     and b.flag_calccust != 'N'
					where a.kg_realizado > 0 and year(a.data_pr) = '" .$ano_2 . "'" .$p01 . "
					group by  b.linha
					order by b.linha");
					$ano_1 = $ano_2;					
...
<table width="99%" border="0">
      <tr>
        ...
        <th align="center" ><h1>Custo Linhas de Produtos por Ano - <?php echo($ano_2); ?> </h1></th>
...
<table width="99%" border="0">
            <tr >
              <th  align="center">
              <select name="mes_i" class="search-input7" >
              ...
              <th  align="center">Ano                
              <input type="text" name="ano_2" id="ano_2" maxlenght="4" size=8  value = "<?php echo($ano_2);?>"  class="search-input5" />
              </th>
              <th  align="center">
              <input name="Pesquisar" type="submit" value="Pesquisar" class="search-submit2">
              <input type="button" onClick="sair2();" value="Sair" class="search-submit2">
              </th>
            </tr>           
</table>     
...
///////////////		  
	...
	/*
		   ...
		   $rs_resumocusto = "CREATE TABLE tmp_resumocusto AS
					 ...
					 and a.cod_prod not in (190000,199481)
									 and mid(a.cod_prod,1,2)  <> 10 and a.motivo != 3
									 group by a.cod_prod,year(a.data_saida), month(a.data_saida),b.cod_prod
					union all
					select a.cod_prod as cd_matprima,year(a.data_saida) ano_saida,
									 ...
									 left outer join tb_custoprod d on d.cod_prod = a.cod_prod and d.ano_custo = year(a.data_saida) and d.mes_custo = month(a.data_saida)
										 where year(a.data_saida)= '" . $ano_2 . "'". $p03 . " and month(a.data_saida) = month(a.data_saida)
					 and a.cod_prod in (190000,190481)
									 and mid(a.cod_prod,1,2)  <> 10 and a.motivo != 3
									 group by a.cod_prod,year(a.data_saida), month(a.data_saida),b.cod_prod ";
...
//					 and a.cod_prod in (190045,190105)
	...
	/////////////Geração da saida para o custo mensal por linha pesquisada //////////////////////////////////////////////////////////////////
	       ...
	       $rs2 = mysql_query("select a.ano_saida,a.mes_saida,sum(a.total_custo) total_cmprim,
							   a.cod_prod,a.linha,b.total_prod ,(sum(a.total_custo)/b.total_prod ) tot_mprimkg, c.tot_insumomes,
							   (c.tot_insumomes / b.total_prod) totcusto_kg, d.valor_dolar , ((c.tot_insumomes / b.total_prod) / d.valor_dolar ) insumos_kg_dolar,
							   ((sum(a.total_custo)/b.total_prod )/ d.valor_dolar ) tot_mprimkg_dolar,
		...
		if ($gera_saida == "S"){ 
		  ...
		  echo("<table width='99%'><tr  bgcolor='#D2D2FF' >");	  
		  ...
		  while($row=mysql_fetch_array($rs2)){
			  ...
			  if ($mes_soma != $row['mes_saida'] ){
				 ...
				 if ($totg_prodm > 0 ){
				   ...
				   echo("<tr><th colspan=4 align=right>T o t a i s  d o  M ê s ....:</th>");
				  
				  
				    echo("<th align='right'>" . number_format($totg_prodm,3,",",".") . "</th>");
					echo("<th align='right'>" . number_format($totg_mprm,2,",",".") . "</th>");
					echo("<th align='right'>" . number_format($totg_insm,2,",",".") . "</th>");
					echo("<th align='right'>" . number_format($totg_ctindm,2,",",".") . "</th>");
					echo("<th align='right'>" . number_format($totg_cttotm,2,",",".") . "</th>");
				
					if ($totg_insm > 0){
					    echo("<th align='right'>" . number_format((($totg_ctindm / ($totg_mprm + $totg_insm )*100)) ,2,",",".") . "%</th>");
					}
					echo("<th >&nbsp;</th>");
					
					echo("<th align='right' colspan='2'>");
				 }
					  $totg_prodm = 0.00;
					  $totg_mprm = 0.00;
					  $totg_insm = 0.00;
			  ...
			  $rs_nmpr = mysql_query("Select a.descr_prod from tb_produto a where a.cod_prod = '" . $row['cod_prod'] . "' ");
	          $row_nmpr = mysql_fetch_assoc($rs_nmpr);
	 	      $nmpr =  $row_nmpr['descr_prod'];
            
			  echo("<th align='left'>" . $row['cod_prod'] . " - " . $nmpr . "</th>");
			  echo("<th align='center'>" . $row['linha'] . "</th>");
			  echo("<th align='right'>" . number_format($row['total_prod'],3,",",".") . "</th>");

			  $totg_prodm = $totg_prodm + $row['total_prod'];			  
			  ...
			  $totg_mprm = $totg_mprm + $row['total_cmprim'];
			  ...
			  $totg_insm = $totg_insm + $row['tot_insumomes'];
			  ...
			  $totg_ctindm = $totg_ctindm + $row['custo_ind_prod'];
			 ...
			 // $ttcustoind = $ttcustoind + $row['custo_ind_prod'];
			  ...
			  $totg_cttotm = $totg_cttotm + $row['total_custo'];
		...
		////////////////////////////////////////////////////////////////////////////////////////////////
						  ...
						  echo("<tr><th colspan=4 align=center>T o t a i s  d o  M ê s : ---->>>></th>");
		...
		echo("<th colspan='4' align='center'> Custos Totais --->>> </th>" );
		...
		echo("<th align='right'>" . number_format((($totg_ctind / ($totg_mpr + $totg_ins )*100)) ,2,",",".") . "%</th>");
		echo("<th >&nbsp;</th>");
	  ...
	  /*
		...
		*/
		...
		echo("<th align='right'>" . number_format((($ttger_ctind / ($totgg_mpr + $totgg_ins )*100)) ,2,",",".") . "%</th>");
		echo("<th >&nbsp;</th>");
