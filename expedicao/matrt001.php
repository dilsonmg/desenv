...
if (isset($codigo_matp2)){
	if ($codigo_matp2 <> "" ){
        if(!is_numeric($codigo_matp2)){
		     $p1 = " and b.descr_prod like '%". $codigo_matp2 ."%'" ; 
		     $p11b = " and b.descr_prod like '%". $codigo_matp2 ."%'" ;
			 $habilit = "S"; 
		}
		else{
			 $p1 = " and b.cod_prod = '" . $codigo_matp2 ."'" ; 
			 $p11a = " and b.cod_prod = '" . $codigo_matp2 ."'"; 
			 $p3 = " and a.cod_prod = '". $codigo_matp2 ."'" ;
			 $habilit = "S";
...
from tb_entmatp a
 inner join tb_produto b on a.cod_prod = b.cod_prod ". $p11b ." 
 inner join tb_fornecedor  c on c.cod_fornec = a.cod_fornec
 where a.id_entmatp > 0 " .$p2 . $p11a ."
...
<tr>
        ...
        Informe o Codigo ou Nome Mat&eacute;ria prima
<input type="text" id = "codigo_matp2"  name="codigo_matp2"  maxlength="45" size="42" placeholder="informe codigo ou nome"  value="" class="search-input4">
         Lote M.Prima
          <input type="text" id = "lote_fabr2"  name="lote_fabr2"  maxlength="45" size="42" placeholder="informe o lote !"  value="" class="search-input4">
...
<?php
    ...
    //busca os dados do lote fabricado	  
      ...
      $rs200 = mysql_query("SELECT a.*,b.descr_prod FROM tb_saidmatp a
               inner join tb_produto b on a.cod_prod = b.cod_prod " . $p11b . "
               where a.cod_prod > 0 " .$p3 . " order by a.data_saida desc ");
...
//echo($b);
     ...
     <br>  <h4><font color="#666666"> Lotes Fabricados com a Materia-Prima - <?php echo ($cod_prod . " - " . $desc_prod );
	  if ($lote_fabr2 != ''){
	    echo(" Lote: " . $num_lote_mat);
	  }
		?>
...
///	
 ...
 <a href="javascript:ver_aditivo('matrt000.php?num_lote=<?php echo ($row['lote_fabricado']);?>');"><?php echo ($row['cod_prod'] . " - " . $row['descr_prod']);?></a></td>
           ...
           <td align="center" ><?php echo ($row['unidade']);?></td>
           <td align="center"><?php echo ($row['lote_fabricado']);
		   
		   $rsx = mysql_query("SELECT distinct a.cod_prod, b.descr_prod FROM tb_saidaprodac a
				  left outer join tb_produto b on a.cod_prod = b.cod_prod
				  where num_lote = '" . $row['lote_fabricado'] . "'");
		  $rowxx=mysql_fetch_array($rsx);
		  echo (" - " . $rowxx['descr_prod']);	   
