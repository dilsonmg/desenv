...
*/
		...
		$rs_tmp1 = "create table tmp_mediasaida as 
		           (select a.cod_prod,max(year(a.data_saida)) ano_saida,
		               sum(a.quantid_said) total_saida,
                       IF(b.unid_mat != 'CM' and b.linha = 'M.PRIMAS',avg(a.quantid_said)/1000,avg(a.quantid_said)) media_ano, b.unid_mat,
					   b.saldo_prod
                    from tb_saidmatp a
					inner join tb_produto b on a.cod_prod = b.cod_prod
					group by a.cod_prod)";
							
		$tmp =  mysql_query( $rs_tmp1 );
...
if (isset($id_subgrupo1)){
	if ($id_subgrupo1 <> "" ){
			$p31a = " and a.id_subgrupo = '" . $id_subgrupo1 . "'"; }
}
...
if (isset($data_1)){
	if ($data_1 <> "" ){
		 $p300 = " and a.data_saida >= '" .  str_replace("/","",formata_data2($data_1)) ."'" ; 
		 $dp1 = $data_1;
		 $dtp1e = $data_1;
...
if (isset($data_2)){
	if ($data_2 <> "" ){
		 $p400 = " and a.data_saida <= '" .  str_replace("/","",formata_data2($data_2)) ."'" ; 
		 $dp2 = $data_2;
		 $dtp2e = $data_2;
...
//		 $p1 = " and a.data_venc >= '" . formata_data2($data_1) ."'" ; 
		 $p1 = " and w.data_entrada >= '" . $data_1 ."'" ; 
		 $p11 = " and v.data_saida >= '" . $data_1 ."'" ; 
...
if (isset($data_2)){
	if ($data_2 <> "" ){
		 ...
		 $p2 = " and w.data_entrada <= '" . $data_2 ."'" ; 
		 $p21 = " and v.data_saida <= '" . $data_2 ."'" ; 		 
...
if (isset($data_3)){
	if ($data_3 <> "" ){
		 $p31 = " and a.data_venc >= '" . $data_3 ."'" ; 
		 $d3 = $data_3;	
		 }}

if (isset($data_4)){
	if ($data_4 <> "" ){
		 $p32 = " and a.data_venc <= '" . $data_4 ."'" ;
		 $d4 = $data_4; 		 
		 }}
...
$rs20 = mysql_query("select a.* , DATEDIFF(a.data_venc,CURDATE()) dias_avencer,
                    ...
                    left outer join tv_ttsaidmatp v on v.cod_prod = a.cod_prod and v.num_lote = a.num_lote and v.data_saida = a.data_entrada
					 " . $p11  . $p21 ."
                     where a.id_entmatp > 0 " . $p3 . " and a.cod_prod not in (100010,100011,100012) 
					 group by a.cod_prod, a.num_lote,a.num_nf
                    order by a.cod_prod asc");				  
...
$rs2 = mysql_query("select a.* , DATEDIFF(a.data_venc,CURDATE()) dias_avencer,
                    ...
                    DATE_FORMAT(a.data_venc, '%d/%m/%Y') data_vencf, b.descr_prod,c.cod_fornec,c.rz_social,
                    d.tt_entradalote,d.tt_saidalote,d.sald_lote,
                    if(d.sald_lote is null,d.tt_entradalote,d.sald_lote) tt_lote, w.quantid_ent, v.quantid_said,e.descr_subgr
                    from tb_entmatp a  
					...
					left outer join tb_subgrupo    e on e.id_subgrupo = a.id_subgrupo 
					left outer join tv_ttentmatp w on w.cod_prod = a.cod_prod and w.num_lote = a.num_lote 
					 " . $p1  . $p2 ."
                    left outer join tv_ttsaidmatp v on v.cod_prod = a.cod_prod and v.num_lote = a.num_lote 
					 " . $p11  . $p21 ."
                     where a.id_entmatp > 0 " . $p3 . $p31 . $p31a . $p32 ." and a.cod_prod not in (100010,100011,100012) 
					 group by a.cod_prod, a.num_lote,a.num_nf,v.data_saida,w.data_entrada
                    order by b.descr_prod,a.data_venc asc ");				  
...
if (isset($dtp1)){
		// $p5e = " and a.data_entrada = '" . str_replace("'","",str_replace("/","-",formata_data2($dtp1))) ."'"  ; 
...
if (isset($dtp1e)){
	if ($dtp1e <> ""  ){
		 $p3e = " and a.data_entrada >= '" . str_replace("/","",formata_data2($dtp1e)) ."'" ; 
	 }
}else{
...
if (isset($dtp1e)){
	if ($dtp1e <> ""  ){
		 $p4e = " and a.data_entrada <= '" . str_replace("/","",formata_data2($dtp2e)) ."'" ; 
	 }
}else{
		 $p3e = " and a.data_entrada = '" . $maior_entrada ."'" ; 
...
<table width="100%" border="0">
      <tr>
        ...
        <th align="center"><h2>Vencimentos de Mat&eacute;rias-primas em : <?php echo($hoje); ?></h2>
        <br> <h2><?php if ($p31 != "" || $p32 != ""   ) { echo("Período de Vencimentos Pesquisado de: " . formata_data5($d3) . " a " . formata_data5($d4)); } ?></h2>
        </th>
      ...
      <tr>
        <th colspan="3" align="center">M. Prima
          <input type="text" id = "mprimaps" name="mprimaps"  maxlength="40" size="30" class="search-input4" >
          Periodo
...
Linha
        <select name="mlinha"  class="search-input4">
          <option value="" >Todas</option>
          <option value="EMBALAGEM"  <?php if($mlinha == 'EMBALAGEM' ) {?> selected <?php }?>> Embalagens </option>
          <option value="M.PRIMAS"  <?php if($mlinha == 'M.PRIMAS' ) {?> selected <?php }?>> Matéria-Prima </option>
        </select>
           Tipo
          <select name="id_subgrupo1" class="search-input3">
     ...
     Vencimento
          <input type="date" name="data_3" size="10" maxlength="10"  title="Informe no Formato 99/99/9999" onkeypress="mascara(this)" onblur="verifica_data(this.value,data_3);"/>
a
<input type="date" name="data_4" size="10" maxlength="10"  title="Informe no Formato 99/99/9999" onkeypress="mascara(this)" onblur="verifica_data(this.value,data_4);"/>

<input type="submit"  name="Pesquisar"   value="Filtrar" onClick="refazform();"  class="search-submit2" />
...
<tr >
            ...
            <tr align="center">
              ...
              <table width="100%" border="1">
                <tr bgcolor="#D2D2FF" >
                  <th width="15%" rowspan="2" >Fornecedor</th>
                  <th width="4%" rowspan="2" >Codigo</th>
                  <th width="9%" rowspan="2" >Produto</th>
                  <th width="3%" rowspan="2" >Unidade</th>
                  <th colspan="2"  bgcolor="#A8A8FF">Nota Fiscal</th>
                  <th colspan="3"  bgcolor="#9DFF9D">Lote / Partida</th>
                  <th width="4%" rowspan="2">Atv Kamoran</th>
                  <th width="4%" rowspan="2">Inicial</th>
                  <th width="4%" rowspan="2">Entrada</th>
                  <th width="5%" rowspan="2">Saida</th>
                  <th width="5%" rowspan="2">Saldo KG</th>
                  <th width="15%" rowspan="2">Obs</th>
                  <th width="15%" rowspan="2">Tipo</th>
                </tr>
                <tr bgcolor="#D2D2FF" >
                  <th width="5%" >Numero</th>
                  <th width="4%" >Data</th>
                  <th width="7%" >N. Lote</th>
                  <th width="4%" >Dt. Fab.</th>
                  <th width="12%" >Dt. Venc.</th>
                </tr>
     <?php
//echo($b);
	 if ($b > 0){
	  ...
	  if($row['tt_lote'] > 0.000 ){
       ...
       if ($row['cod_prod'] != $cd_prodini && $tot_prod > 0){  ?>
                ...
                <tr bgcolor="#E6FFE6">
                  <td colspan="11" align="center" >T O T A L </td>
                <td align="right" ><?php  
				  if ($msk == "S") {
                ...
                <td align="right"><?php 
				 ...
				 ?></td>
                <td align="left" bgcolor="#E6FFE6">
                 <?php
				    $nmes_supr = 0;
				    if ($tot_prod > 0) {
					
					
				      	$rs20xy = mysql_query("select a1.media_ano from  tmp_mediasaida a1
                                      where a1.cod_prod = '" . $cd_prodini ."'");
				        $rowxy2 = mysql_fetch_array($rs20xy);
					     
						 
						if($rowxy2['media_ano']>0){
						    $nmes_supr = $tot_prod / $rowxy2['media_ano'];
                            echo("Meses suprimento : " . number_format($nmes_supr,2,',',''));
...
//		      if ($row['msg_lote'] <> ""){ echo("<br> <font color=#999999> " . $row['msg_lote'] . "</font>");
	///		  }
		  ...
		  */
		   ...
		   ?></td>
           <td align="left"><?php echo ($row['msg_lote']);?></td>
           <td align="center"><?php echo ($row['descr_subgr']);?></td>
              </tr>
          <?php 
		   } 
	      }
		  ...
		  ?>     
             ...
             <table width="100%">
                <tr bgcolor="#003366"> 
                  <td align="center"><font color="#FFFFFF" size="+1"> Sa&iacute;das - Mat&eacute;rias-primas utilizadas no Per&iacute;odo </font></td>
                </tr>
             </table>
    <table width="100%" border="1">
                 <tr bgcolor="#D2D2FF" >
...
<br>
            <table width="100%">
                <tr bgcolor="#003366"> 
                  <td align="center"><font color="#FFFFFF" size="+1">Entradas de Matérias-primas  </font></td>
                </tr>
             </table>
          <table width="100%" border="1">
                <tr bgcolor="#D2D2FF" >
