...
$hoje = date("d/m/Y");
/*
SELECT  a.*,b.descr_despreal,c.id_grupocusto,d.descr_grupocc,"P"  FROM tb_prevdespr a
 							inner join tb_despreal b on b.cod_sidespreal = a.cod_sidespreal
							inner join tb_vinccustor c on a.cod_sidespreal = c.cod_sidespreal
              inner join tb_grupoccusto d on d.id_grupocusto = c.id_grupocusto
              where a.cod_sidespreal > 0 and ano_prevdespr = 2023
							      group by a.ano_prevdespr,a.mes_prevdespr,a.cod_sidespreal
union all
SELECT a.id_custoreal,a.cod_sidespreal,a.mes_custoreal,a.ano_custoreal,a.val_custoreal
       ,b.descr_despreal,c.id_grupocusto,d.descr_grupocc, "R"  FROM tb_custoreal a
							...
							inner join tb_vinccustor c on a.cod_sidespreal = c.cod_sidespreal
              inner join tb_grupoccusto d on d.id_grupocusto = c.id_grupocusto
              where a.cod_sidespreal > 0 and ano_custoreal = 2023
							      group by a.ano_custoreal,a.mes_custoreal,a.cod_sidespreal
order by 7,2,9,3,4
*/
...
if(isset($_POST["botao"])){
	 ...
	 while($row=mysql_fetch_array($rs2)){ 
	   ...
	   //echo('<tr ' . $bgc .'>');
	   if ( $cd_real0 != $header2[0] ){ 
     
	 //////////////imprime linha ///////////////
	    echo("<tr>");
	  	  for ($i=0;$i < 16; $i++) {
			  if($i==0 &&  $tt_lin[14] > 0){$alg = " align=right ";
			    $tt_lin[1] = "Budget ";
			  }else{$alg=" align=right ";}
			  if ($tt_lin[$i] > 0 && $i > 1) {
			      echo("<th" . $alg . ">" . number_format(abs($tt_lin[$i]),2,",",".") ."</th>" );
			  }else {
				  // if($tt_lin[2] >0){
	   	  ...
	   	  for ($i=0;$i < 16; $i++) {
			  ...
			  if ($header2[$i] > 0 && $i > 1) {
		          ...
		          if ($row['P'] == 'P'){
				       ...
				       $tt_lin[$i] = abs($header2[$i] - $tt_lin[$i])  ;					  
				  }
			  }else{
