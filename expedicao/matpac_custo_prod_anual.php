	...
	if($linha_prod != ""){
	   ...
	   $rs_resumocusto = "CREATE TABLE tmp_resumocusto AS
							select a.cod_prod as cd_matprima,year(a.data_saida) ano_saida,
								 ...
								 where year(a.data_saida)= '" . $ano_2 . "' and month(a.data_saida) = month(a.data_saida)
                 and a.cod_prod not in (190045,190105)
								 and mid(a.cod_prod,1,2)  <> 10 and a.motivo != 3
								 group by a.cod_prod,year(a.data_saida), month(a.data_saida),b.cod_prod
				union all
...
<table width="99%" border="0">
      <tr>
        ...
        <th align="center" ><h1>Consulta Custo Anual por Linha de Produto- 
        <?php
	echo($ano_2);
		  ?>
      </h1></th>
...
<input type="text" name="ano_2" id="ano_2" maxlenght="4" size=8 onChange="atualiza()"  value = "<?php echo($ano_2);?>"  class="search-input5" /></th>
    ...
    <?php if ($gera_saida == "S"){ 
	  ...
	  while($row=mysql_fetch_array($rs2)){
		  ...
		  echo("<td align='right'>" . number_format($row['total_custo'],3,",",".") . "</td>");
		 ...
		 // $ttcustoind = $ttcustoind + $row['custo_ind_prod'];
		  ...
		  echo("<td align='right'>" . number_format(($row['custo_ind_prod']/$row['total_prod']),3,",",".") . "</td>");
		  
		  $totg_kgre = $totg_kgre + $row['total_custo'];
		  echo("<td align='right'>" . number_format($custo_ind_prod ,3,",",".") . "</td>");

		  echo("<td align='right'>"); 
		    if($row['total_custo_kg_real'] > 0){
		        echo(number_format($row['total_custo_kg_real'],3,",","."));
			}
			else{
				$tt_custkg = ($row['custo_ind_prod'] + $custo_ind_prod);
		        echo(number_format($tt_custkg,3,",","."));
		    ...
		    echo("<td align='right'>");
		    if ($row['total_custo_kg_real'] > 0){
		      echo(number_format(($row['total_custo_kg_real'] / $row['valor_dolar']) ,3,",","."));
			}else{
			  echo(number_format(($tt_custkg / $row['valor_dolar']) ,3,",","."));
		  ...
		  echo("<td align='right'>" . number_format($row['valor_dolar'],3,",",".") . "</td>");
