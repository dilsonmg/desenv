...
$rs2 = mysql_query("SELECT a.cod_prod,b.descr_prod, year(a.data_saida) ano_said,month(a.data_saida) mes_saida,
                    ...
                    FROM tb_saidmatp a
                    inner join tb_produto b on b.cod_prod = a.cod_prod
					where year(a.data_saida) > 2018 " . $p1 . $p2 . $p3 ."  and a.cod_prod not in (100010,100011,100012) 
					group by ano_said, mes_saida,a.cod_prod
...
<table width="100%" border="1" cellspacing="0"   align="center"   >
  ...
  <select id="lote_fabricado" name="lote_fabricado" onChange="atualiza();" class="search-input5 " >
            <option value="">Selecione o produto Fabricado</option>
            <?php while($row3=mysql_fetch_assoc($rs3)){ ?>
            <option value="<?php print($row3['num_lote'])?>"
				  <? if($row3['num_lote'] == $lote_fabricado ) {?>selected <? } ?>				
				 ><?php print($row3['num_lote'] . " - " .$row3['descr_prod'] . " - " . $row3['cod_prod'] )?></option>
            <?php }?>
        </select>
