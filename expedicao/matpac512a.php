...
$rs2 = mysql_query("select a.*
					...
					,DATE_FORMAT(a.dt_fat, '%d/%m/%Y') data_fatura
					...
					left outer join tv_percfret c on a.num_conhec = c.num_conhec and c.perc_fretfr > 4
...
<![endif]-->
	<title>MATPA512 - Frestes para aprovação</title>
...
<table width="99%" border="0">
      <tr>
        ...
        <th align="center" ><h1>Consulta de Fretes 
          - 
          <?php
	echo($hoje);
		if($data_1 !="" || $data_2 != ""){
...
//echo($b);
	...
	//	 $percfat = ($tot_fr / $tot_nf) * 100 ;
         ...
         <th colspan=2><h3> Total NF:a".number_format($tot_nf,2,',','.') . "</h3></th>
     ...
     if($bg == 1){
	   ...
	   echo('<tr ' . $bgc .'>');?>
		 ...
		 echo ("<tr bgcolor=#E9E9E9 valign='middle'>
		 <th colspan=2><h3> N. CTE:".$cte_l . "</h3></th>
         <th colspan=2><h3> Total NF: b".number_format($tot_nf,2,',','.') . "</h3></th>
         <th colspan=2><h3> Total Frete:".number_format($tot_fr,2,',','.') . "</h3></th>
		 <th colspan=2><h3> % ICMS:".number_format($aliq_icms,2,',','.') . "%</h3></th>
		 <th colspan=1><h3> Valor ICMS:".number_format($icms,2,',','.') . "</h3></th>
		 <th colspan=5><h3> % Frete:".number_format($percfat,2,',','.') . "% " . $bl . "</h3></th>
