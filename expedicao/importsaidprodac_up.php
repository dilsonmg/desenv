...
/*================================ importar dados de produtos ==============================*/
     ...
     <?
		...
		*/
        ...
        if ($a > 0) {
			$altera = "update tb_saidaprodac set unidade = '" . $data[1] . "'," .
			" obs_bonif = '" . $obs_bonif . "'," .
			" sigla_bonifi = '" . $data[11] . "'" .
			" where cod_prod = ". intval($data[0]) 
		       ." and num_lote   = '" . trim($data[2]) 
			   ."' and num_pedido  = " . intval($data[4])
			   ." and num_nf      = " . intval($data[5]) 
			   ." and data_nf     = '" . $data[6]
			...
			$inclui = " insert into tb_saidaprodac (id_saidaprodac,cod_prod,unidade,num_lote,quantid,num_pedido,
			            num_nf,data_nf,codigo_cli,data_fatura,cod_fornec,obs_bonif,sigla_bonifi) 
			values ('$nvid','$data[0]','$data[1]','$ddt2','$data[3]','$data[4]','$data[5]','$data[6]','$data[7]','$data[8]','$data[9]','$obs_bonif','$data[11]')";
			//echo($inclui);
