	   ...
	   */	
       if ($mes_i > 0) {
		   ...
		   $rs_tbtemp2 = "create table tmp_totkgprod as (
						  ...
						  and month(a.data_pr) = '" . $mes_i . "' and a.lote_realizado != ''
						  group by a.cod_prod, b.linha,year(a.data_pr),month(a.data_pr)
						  order by year(a.data_pr) desc,month(a.data_pr),b.linha,b.cod_prod  )";
          // echo($rs_tbtemp2);						  
...
group by a.cod_prod,b.cod_prod order by a.cod_prod, year(a.data_saida), month(a.data_saida),b.cod_prod
		   ...
		   SELECT a.*,
 STR_TO_DATE(a.data_fabr, '%d/%m/%Y') AS data_fabrfom,
       MONTH(STR_TO_DATE(a.data_fabr, '%d/%m/%Y')) AS mesfabr
FROM tb_entprodac a;
...
Ano
<input type="text" name="ano_2" id="ano_2" maxlenght="4" size=8 onChange="atualiza()"  value = "<?php echo($ano_2);?>"  class="search-input2"  /></th>
              ...
              <th  align="center"><input name="Pesquisar" type="submit" value="Pesquisar" class="button">
...
//echo($totcol);
	  ...
	  while($row = mysql_fetch_array($saidas_mat)){ 
		...
		if( $mat_inic != $row['cd_matprima']){
					...
					$rs_mprima = mysql_query("select a.* from tb_produto a where a.cod_prod = '" . $row['cd_matprima'] . "' and a.flag_calccust != 'N' ") ;
					$row1m=mysql_fetch_array($rs_mprima);
